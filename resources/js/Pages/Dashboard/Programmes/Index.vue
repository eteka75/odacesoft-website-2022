      ...
      <template #btn>
        <div>
          <button
            ...
            @click="openModal('add', 'Nouvelle programme')"
          >
            <span class="pe-2">
                ...
                <div class="modal-body">
                    ...
                    <div class="col ">
                      <div class="form-floating">
                        <input
                          ...
                          :class="[
                            $page.props.errors.titre ? 'is-invalid' : '',
                          ]"
                          placeholder="Le nom du programme"
                          v-model="form.titre"
                        />

                        <label for="categ">Le nom du programme (*)</label>
                      </div>
                      <div
                    ...
                    <!--div class="col">
                      <div class="form-floating">
                        <select  v-model="form.type"  :class="[
                            $page.props.errors.type ? 'is-invalid' : '',
                          ]" class="form-select">
                          <option value="TEXTZ">Texte</option>
                        ...
                        <label for="categ">Type de fichier (*)</label>
                      </div>
                      <div
                  ...
                  <div class="form-group mb-3">
                      <div class="form-floating">
                        ...
                        <label for="categ">Description du programme(*)</label>
                      </div>
                      <div
              ...
              <div class="card-body p-4">
                <div class="mb-3 d-flex py-3">
                    ...
                    <button
                        ...
                        v-show="!editMode"
                        @click="save(form)"
                        :disabled="form.processing || form.titre==null || form.type==null"
                    >
                  ...
                  <tbody v-if="datas.data.length > 0">
                    <tr  @click="ShowInfo(row)" :class="(sdata && row && row.id==sdata.id)?'bg-primary text-white':''"  v-for="row in datas.data" v-bind:key="row">
                      ...
                      <td data-label="Job Title">
                        <div
                          class=" bg-none p-0 font-bold d-block"
                          @click="ShowInfo(row)"
                        >
                          {{ row.title }}
                        </div>
                        <p :class="(sdata && row && row.id==sdata.id)?'text-white':'text-muted'" >{{liveSubstr(row.resume,0,46)}}</p>
                      <small><i class="fas fa-calendar-alt"></i> {{ row.h_created_at }}</small>
                      </td>
                      <td >
                        <span v-if="row.actif" class="badge bg-success rounded-5 text-white">Actif</span>
                        <span v-if="!row.actif" class="badge  rounded-5  text-white" :class="(sdata && row && row.id==sdata.id)?'bg-white':'bg-gray-500'" >Désactivé</span>
                      </td>
                      
                      <td nowrap class="text-center">
                              <button @click="edit(row)"  :class="(sdata && row && row.id==sdata.id)?'text-white':'text-dark'" class="mx-1 bg-none">
                                <i class="fas fa-edit"></i>
            ...
            <div v-if="sdata" class="card shadow-1 mt-3">
              <div class="card-body">
                <div class="row text-xs">
                  ...
                  <h5 class="card-header h4 mb-3 px-3 py-1 border-0 text-center text-uppercase">{{ sdata.title }}</h5>
                  <div class="col-md-4 text-mute font-bold text-end">

                  </div>
                  <div class="col-md-8">
                    <img v-if="sdata.img" class="img-responsive rounded-3 border-1 shadow-sm w-24 mb-3" :src="'/'+sdata.img" :alt="sdata.img">
  ...
  methods: {
    ...
    save: function (data) {
      ...
      //this.$inertia.post(route('tags.store'), data);
       if (this.ClickEditor() ) {
        this.$inertia.post(route("programme.store"), data, {
    ...
    update: function (data) {
      data._method = "PUT";
      var url = route("programme.update", data.id);
      if (this.ClickEditor() ) {
    ...
    deleteRow: function (data) {
      ...
      data._method = "DELETE";
      this.$inertia.post(route("programme.destroy", data.id), data);
      this.reset();
      this.closeModal();
    },
    selectImage() {
