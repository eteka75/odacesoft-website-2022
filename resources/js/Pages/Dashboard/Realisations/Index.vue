                ...
                <div class="modal-body">
                    ...
                    <div class="col ">
                      <div class="form-floating">
                        <input
                          ...
                          placeholder="Le nom de la réalisation"
                          v-model="form.nom"
                        />

                        <label for="real">Le nom de la réalisation</label>
                      </div>
                      <div
                  ...
                  <div class="form-group mb-3">
                      <div class="form-floating">
                        ...
                        <label for="part">Description de la réalistion (*)</label>
                      </div>
                      <div
            ...
            <div class="card rounded-2 shadow-1 mb-5">
              <div class="card-body p-4">
                ...
                <div class="row">
                  ...
                  <div
                                    v-if="$page.props.errors.categorie_id"
                                    class="text-danger py-1 text-xs"
                                  >
                                    {{ $page.props.errors.categorie_id }}
                                </div>
                </div>
              ...
              <div class="card-body p-4">
                <div class="mb-3">
                   <label for="formFileSm" class="form-label"
                    >Ajouter une image de la réalisation </label
                  >
                ...
                >
                  ...
                  <tbody v-if="datas.data.length > 0">
                    <tr  @click="ShowInfo(row)"  :class="(sdata && row && row.id==sdata.id)?'bg-primary text-white':''"  v-for="row in datas.data" v-bind:key="row">
                      <td> {{ row.id }}</td>
                      <td v-if="row" data-label="Job Title">
                        {{ liveSubstr(row.title,0,50) }}
                      </td>
                      <td >
                         <span v-if="row.service">{{ row.service.title }}</span>
                      </td>
                      <td class="text-xs" >
                      ...
                      <td data-label="" class="text-center">
                              ...
                              <button @click="edit(row)" class="d-inline mx-1 bg-none" :class="(sdata && row && row.id==sdata.id)?' text-white':''">
                                <i class="fas fa-edit"></i>
                                
                              </button>

                              <inertia-link
                                :href="route('realisation.show',row.id)"
                                class="d-inline  mx-1 bg-none" :class="(sdata && row && row.id==sdata.id)?' text-white':'text-dark'"
                              >
                              ...
                              @click="deleteRow(row)"
                              class="d-inline mx-1 bg-none text-danger"
                              >
            ...
            <div v-if="sdata" class="card shadow-1 mt-3">
              <div class="card-body">
                <div class="row text-xs">
                  ...
                  <h5 class="card-header border-0 h4 mb-3 px-3 py-1 text-uppercase txt-center">{{ sdata.title }}</h5>
                  <div class="col-md-4 text-mute font-bold text-end">

                  </div>
                  <div class="col-md-8">
                    <img v-if="sdata.img" :src="'/'+sdata.img" class="w-24 img-responsive_ rounded-3 mb-3 shadow-sm" :alt="sdata.img" />
                  </div>
                  <div class="col-md-4 text-mute font-bold text-end">
                    Nom :
                  </div>
                  <div class="col-md-8">{{ sdata.title }}</div>
                  <div class="col-md-4 text-mute font-bold text-end">
                    Resume :
                  </div>
                  <div class="col-md-8">{{ sdata.lead }}</div>
                  ...
                  <div class="col-md-12 text-xs pb-5" style="max-height:460px; overflow:auto" v-html="sdata.content">
  ...
  methods: {
    ...
    save: function (data) {
      ...
      //this.$inertia.post(route('tags.store'), data);
       if (this.ClickEditor() ) {
        this.$inertia.post(route("realisation.store"), data, {
    ...
    edit: function (data) {
      ...
      this.editMode = true;
      this.active = data.actif;
      this.previewImage = "/"+data.img;
      this.form.image = data.img;
      this.form.nom= data.title;
      this.form.resume= data.lead;
      this.form.contenu= data.content;
      this.form.categorie_id= data.service_id;
      this.form.fichier= data.img;
    ...
    update: function (data) {
      data._method = "PUT";
      var url = route("realisation.update", data.id);
      if (this.ClickEditor() ) {
