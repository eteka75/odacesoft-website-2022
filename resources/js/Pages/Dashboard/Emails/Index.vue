<template>
  ...
  <div>
    <Top-Header :titre="titre">
      <template #btn>
        <div>
          <inertia-link :href="route('email.create')" class="btn btn-sm btn-success">
            <span class="pe-2">
                ...
                <tbody v-if=' datas.data.length > 0'>
                  ...
                  <td>{{ row.id }}</td>
                    <td data-label="Job Title">
                      ...
                      <inertia-link class="text-dark " :href="route('email.show',row.id)">{{ row.objet }}</inertia-link>
                        <p clas="text-xs text-muted">{{row.destinataire}}</p>
                    ...
                    <td data-label="" class="text-center">
                      <div class="dropdown ">
                        ...
                        <div class="shadow-2">
                           <inertia-link  :href="route('email.show',row.id)"   class="dropdown-item">
                            <i class="fas fa-eye"></i> Afficher
                          </inertia-link>
                          <inertia-link :href="route('email.edit',row.id)"
                          class="dropdown-item"
                          >
                          <i class="fas fa-edit    "></i>
                            Modifier
                          </inertia-link>
                           <button @click="deleteRow(row)"   class="dropdown-item text-danger">
  ...
  methods: {
    ...
    */
    ...
    deleteRow: function (data) {
      ...
      this.$inertia.post(route('email.destroy', data.id), data);
      this.reset();
      this.closeModal();
    },
  },
