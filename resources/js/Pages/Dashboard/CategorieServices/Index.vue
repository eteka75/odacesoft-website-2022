            ...
            >
              <div class="table-responsive py-3 rounded-sm min-h">
                ...
                >
                  ...
                  <tbody v-if="datas.data && datas.data.length">
                    <tr @click="ShowInfo(row)" v-for="row in datas.data" v-bind:key="row">
                      <td  class="w-4">{{ row.id }}</td>
                      <td data-label="Job Title">
                        <button :title="liveSubstr(row.lead,0,50)"
                          class="text-dark bg-white d-block px-0"
                          @click="ShowInfo(row)"
                        >{{ row.title }}
                        </button>
                      </td>
                        ...
                        <div class="dropdown">
                          ...
                          <div class="dropdown-menu dropdown-menu-end">
                            <div class="shadow-2">
                              ...
                              <button @click="edit(row)" class="dropdown-item">
                                <i class="fas fa-edit"></i>
                                Modifier
                              </button>
            ...
            <div v-if="sdata" class="card shadow-1 mt-3">
              <div class="card-body">
                <div class="row text-xs">
                  ...
                  <h5 class="card-header border-0 text-center h4 mb-3 px-3 py-1 text-uppercase">{{ sdata.title }}</h5>
                  <div class="col-md-4 text-mute font-bold text-end">

                  </div>
                  <div class="col-md-8">
                    <img v-if="sdata.img" class="img-responsive rounded-3 border-1 shadow-sm w-24 mb-3" :src="'/'+sdata.img" :alt="sdata.title">
                  ...
                  <div
                    v-if="h_updated_at &&  sdata.h_created_at != sdata.h_updated_at"
                    class="col-md-4 text-mute font-bold text-end"
                  >
...
export default {
  ...
  mounted() {
    this.AddPlugin();
    this.initDatas();
    this.sdata=this.datas.data?this.datas.data[0]:null;
  ...
  methods: {
    ...
    save: function (data) {
      ...
      //this.$inertia.post(route('tags.store'), data);
       if (this.ClickEditor() ) {
        this.$inertia.post(route("categorie-service.store"), data, {
    ...
    update: function (data) {
      data._method = "PUT";
      var url = route("categorie-service.update", data.id);
      if (this.ClickEditor() ) {
    ...
    deleteRow: function (data) {
      ...
      data._method = "DELETE";
      this.$inertia.post(route("categorie-service.destroy", data.id), data);
      this.reset();
      this.closeModal();
    },
    selectImage() {
