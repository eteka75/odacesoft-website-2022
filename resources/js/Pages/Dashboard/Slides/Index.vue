      ...
      <template #btn>
        <div>
          <button
            ...
            @click="openModal('add', 'Nouveau slide')"
          >
            <span class="pe-2">
                ...
                <div class="modal-body">
                  <div class="form-group d-flex gx-2 mb-4">
                    <div class="col ">
                      <div class="form-floating">
                        ...
                        <label for="real">Titre du slide</label>
                      </div>
                      <div
                  ...
                  <div class="form-group mb-3">
                      <div class="form-floating">
                        ...
                        <label for="part">Description du slide (*)</label>
                      </div>
                      <div
          ...
          <div class="col-lg-4 col-xl-4">
            <div class="card rounded-2 mb-4 shadow-1">
              ...
              <div class="d-flex justify-items-center">
                <div class="form-check mx-auto ms-4 col mt-3 form-switch">
                        <input
                        name="close"
              ...
              <div class="card-body p-4">
                <div class="mb-3">
                   <label for="formFileSm" class="form-label"
                    >Ajouter une image au slide </label
                  >
                ...
                >
                  ...
                  <tbody v-if="datas.data.length > 0">
                    <tr  @click="ShowInfo(row)"  :class="(sdata && row && row.id==sdata.id)?'bg-primary text-white':''"  v-for="row in datas.data" v-bind:key="row">
                      <td>
                       {{ row.id }}
                       <!--<img class="rounded-circle me-2" width="55" height="55" :src="'/'+row.image" :alt="row.image" />-->
                      
                     </td>
                      <td >
                       
                         <span v-if="row.titre" :title="row.titre"><b>{{liveSubstr(row.titre,0,40)}}</b></span>
                        <div><small>{{ row.h_created_at }}</small></div>
                      </td>
                      <td>
                         <div>
                           
                           <span v-if="row.etat" class="badge bg-success rounded-5 text-white" >Actif</span>
                        <span v-if="!row.etat" class="badge bg-gray-500 rounded-5  text-white" :class="(sdata && row && row.id==sdata.id)?'bg-white text-dark':''" >Désactivé</span>
                     </div>
                      </td>
                      ...
                      <td data-label="" class="text-center">
                              ...
                              <button @click="edit(row)" class="d-inline  bg-none" :class="(sdata && row && row.id==sdata.id)?'text-white':'text-dark'">
                                <i class="fas fa-edit"></i>
                              ...
                              <button
                                ...
                                @click="deleteRow(row)"
                                class="d-inline text-danger bg-none"
                              >
            ...
            <div v-if="sdata" class="card shadow-1 mt-3">
              <div class="card-body">
                ...
                <div class="row text-xs">
                  ...
                  <h5 class="card-header border-0 h4 mb-3 px-3 py-1 text-uppercase txt-center">SLIDE OU CARROUSEL</h5>
                  <div class="col-md-4 text-mute font-bold text-end-md-md">
                    Image :
                  </div>
                  <div class="col-md-8">
                    <img v-if="sdata.image" :src="'/'+sdata.image" class="w-100 img-responsive_ rounded-3 mb-3 shadow-sm" :alt="sdata.image" />
                  </div>
                  <div class="col-md-4 text-mute font-bold text-end-md">
                    Titre :
                  </div>
                  <div class="col-md-8">{{ sdata.titre }}</div>
                  <div class="col-md-4 text-mute font-bold text-end-md">
                    Slug :
                  </div>
                  ...
                  <div class="col-md-8">{{ sdata.sous_titre }}</div>
                  ...
                  <div class="col-md-8">
                    <span class=" text-success font-bold" v-if="sdata.etat">Actif</span>
                    <span class="text-danger font-bold" v-if="!sdata.etat">Désactivé</span>
                  </div>
                  <div
                    v-if="sdata.user"
                    class="col-md-4 text-mute font-bold text-end-md"
                  >
                    Auteur :
                  </div>
                  ...
                  <div class="col-md-12 text-xs pb-5" style="max-height:460px; overflow:auto" v-html="sdata.detail">
...
export default {
  ...
  props: ["datas",'close','cats'],
  data() {
    return {
      titre: "Slides du site",
      nb: 0,
      sdata: null,
      previewImage: null,
  ...
  methods: {
    ...
    AddPlugin() {
      ...
      $("#summernote").summernote({
        placeholder: "Rédiger votre cntenu ici ...",
        tabsize: 2,
        height: 300,
    ...
    save: function (data) {
      ...
      //this.$inertia.post(route('tags.store'), data);
       ...
       if (this.ClickEditor() ) {
        this.$inertia.post(route("slide.store"), data, {
    ...
    edit: function (data) {
      ...
      this.editMode = true;
      this.active = data.etat;
      this.previewImage = "/"+data.image;
      this.form.image = data.image;
      this.form.nom= data.titre;
      this.form.resume= data.sous_titre;
      this.form.contenu= data.detail;
      this.form.fichier= data.image;
      this.form.active= data.etat=='1'?true:false;;
       $('#summernote').summernote('code',this.form.detail);
    ...
    update: function (data) {
      data._method = "PUT";
      var url = route("slide.update", data.id);
      if (this.ClickEditor() ) {
    ...
    deleteRow: function (data) {
      ...
      data._method = "DELETE";
      this.$inertia.post(route("slide.destroy", data.id), data);
      this.reset();
      this.closeModal();
    },
    selectImage() {
