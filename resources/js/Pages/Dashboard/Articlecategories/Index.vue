  ...
  <div>
    ...
    <div class="main-content">
      ...
      <div class="col-md-8 max bg-white card ">
                            ...
                            <div class="col me-3">
                             ...
                             <div class="col">
                              <div class="form-floating"  >
                                ...
                                <select required v-model="form.couleur" class="form-select" :class="[$page.props.errors.couleur ?'is-invalid':'is-valid_']">
                                    <option value="bg-primary" >Primary (Blue)</option>
                                    <option value="bg-success" >Succèss (Vert)</option>
                                    <option value="bg-danger" >Danger (Rouge)</option>
                                    <option value="bg-warning" >Alerte (Jaune)</option>
                                    <option value="bg-secondary">Secondaire (Gris)</option>
                                    <option value="bg-tertiary">Tertiaire (Rouge claire)</option>
                                    <option value="bg-info">Info (Rose)</option>
                                </select>
                              ...
                              <div
                                    v-if="$page.props.errors.couleur"
                                    class="text-danger py-1 text-xs"
                                  >
                                    {{ $page.props.errors.couleur }}
                                </div>
                            </div>
                            </div>
                            <div class="form-floating">
                ...
                <tbody>
                  ...
                  <td>{{ row.id }}</td>
                    <td data-label="Job Title">
                      ...
                      <button class="text-dark bg-white d-block bg-light-hove" @click="ShowInfo(row)" ><span class="w-3 d-flex float-start me-2 h-3 mt-1 border rounded-2" :class="row.couleur"></span>{{ row.nom }}</button>
        ...
        <div class="card shadow-1 mt-2">

            <div v-if="sdata" class="card-body">
                <div class="row text-xs">
                    <h5 class="card-header h4 mb-3 px-3 py-1">{{sdata.nom}}</h5>

                    <div class="col-md-5  font-bold text-end">Cétgorie :</div>
                    <div class="col-md-7">{{sdata.nom}}</div>
                     <div class="col-md-5 text-mute font-bold text-end">Slug :</div>
                    <div class="col-md-7">{{sdata.slug}}</div>

                    <div class="col-md-5 text-mute font-bold text-end">Création :</div>
                    <div class="col-md-7"><i class="fas fa-calendar" aria-hidden="true"></i> {{sdata.h_created_at}}</div>
                     <div v-if="sdata.user"  class="col-md-5 text-mute font-bold text-end">Auteur :</div>
                    <div v-if="sdata.user" class="col-md-7"><i class="fas fa-calendar" aria-hidden="true"></i> {{sdata.user.name}}</div>
                    <div v-if="sdata.h_created_at!=sdata.h_updated_at" class="col-md-5 text-mute font-bold text-end"> Dernière modification :</div>
                    <div v-if="sdata.h_created_at!=sdata.h_updated_at"  class="col-md-7"> <i class="fas fa-calendar-check" aria-hidden="true"></i> {{sdata.h_updated_at}}</div>

                     <div class="col-md-5 text-mute font-bold text-end">Description :</div>
                    <div class="col-md-7 text-xs">{{sdata.description}}</div>
                </div>
            </div>
        </div>
        </div>
  ...
  methods: {
    ...
    */
    ...
    save: function (data) {
      ...
      //this.$inertia.post(route('categorie-article.store'), data);
      this.$inertia.post(route('categorie-article.store'),data, {
    ...
    data._method = "PUT";
    var url=route('categorie-article.update', data.id);
    ...
    deleteRow: function (data) {
      ...
      this.$inertia.post(route('categorie-article.destroy', data.id), data);
      this.reset();
      this.closeModal();
    },
  },
